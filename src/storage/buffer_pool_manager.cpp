...
Page* BufferPoolManager::fetch_page(PageId page_id) {
    ...
    if (pages_[frame_id].id_.page_no != INVALID_PAGE_ID) {
        if (pages_[frame_id].is_dirty_) {
            disk_manager_->write_page(pages_[frame_id].id_.fd, 
                                     pages_[frame_id].id_.page_no, 
                                     pages_[frame_id].get_data(), 
                                     PAGE_SIZE);
        }
...
bool BufferPoolManager::unpin_page(PageId page_id, bool is_dirty) {
    ...
    // 1. 在page_table_中查找目标页，若不存在返回false
    if (page_table_.count(page_id) == 0) {
        return false;
    }
    ...
    // 3. 如果pin_count已经为0，返回false
    if (page->pin_count_ <= 0) {
        return false;
    }
    
    // 4. 减少pin_count，如果减少后为0，则将其加入replacer
    page->pin_count_--;
    if (page->pin_count_ == 0) {
        replacer_->unpin(frame_id);
    }
    
    // 5. 如果is_dirty为true，则将page的is_dirty置为true
    if (is_dirty) {
        page->is_dirty_ = true;
    }
